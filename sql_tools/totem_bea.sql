...
match AS (
    SELECT
        ...
        t.placa,
        d.dt_desembarque,
        ROW_NUMBER() OVER (PARTITION BY t.placa, t.data_abate, t.'data'ORDER BY d.dt_desembarque ASC) AS rn
    FROM t_totem_gado t
...
dedup AS (
    SELECT
        ...
        placa, 
        dt_desembarque,
        ROW_NUMBER() OVER (PARTITION BY placa, dt_desembarque, data_abate ORDER BY "data" DESC) AS rk
    FROM pick_next
)
