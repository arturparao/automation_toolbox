...
match AS (
    ...
    LEFT JOIN desembarque d
     ...
     AND d.dt_desembarque >= t."data"   -- use > se quiser estritamente após
),
pick_next AS (
    SELECT data_abate, "data", placa, dt_desembarque
    FROM match
    WHERE rn = 1
),

-- 2) Se várias linhas (mesmo data_abate) baterem no mesmo dt_desembarque,

dedup AS (
    SELECT
        data_abate, "data", placa, dt_desembarque,
        ROW_NUMBER() OVER (
